...
class TestTransactionsDB(TestCase):
    ...
    def test_add_and_remove_category(self):
        ...
        category = CategoryModel(name="public_transport", display_name="Public Transport")
        db.session.add(category)
        db.session.commit()

        # Test that the category has been successfuly created
        self.assertIsNotNone(CategoryModel.query.get("public_transport"))

        # Delete the category
        db.session.delete(category)
        db.session.commit()
        self.assertIsNone(CategoryModel.query.get(category.name))

    def test_adding_conflicting_categories(self):
        """Ensure that we get an exception when adding categories with conficting names"""
        category_1 = CategoryModel(name="public_transport", display_name="Public Transport")
        category_2 = CategoryModel(name="public_transport", display_name="Public Transport")
    ...
    def test_add_category_with_missing_name(self):
        category = CategoryModel(display_name="Investments")
        db.session.add(category)
    ...
    def test_add_category_with_missing_display_name(self):
        category = CategoryModel(name="investments")
        db.session.add(category)
    ...
    def test_add_transaction(self):
        ...
        category = CategoryModel(name="groceries", display_name="Groceries")
        db.session.add(category)
        db.session.commit()
        ...
        result = TransactionModel.query.get(transaction.id)
